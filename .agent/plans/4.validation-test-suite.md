...
## Goal
Create an agent-executable validation checklist saved to `.agent/validation/` that covers all built features (Modules 1 & 2). The checklist uses curl for API tests and agent-browser CLI tools for E2E browser tests, with clear pass/fail criteria.
...
```
### TEST-ID: Description
**Steps:** (curl command or agent-browser CLI tool sequence)
